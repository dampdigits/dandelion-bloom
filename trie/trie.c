// Saves popular dog names in a trie and inputs a dog name to check if it is present in trie
...
// Check if name is present in trie
bool check(char *word)
{
    node *cursor = root;
    int word_length = strlen(word);

    // check if each letter in word is present in trie
    for (int i = 0; i < word_length; i++)
    {
        // find i-th letter
        cursor = cursor->children[tolower(word[i]) - 'a'];

        // check if letter is present in trie
        if (cursor == NULL)
        {
            return false;
        }
